...
var ads = '<div style="padding: 0.3em 0.3em; padding-top: 0;"><div style="background: #3e3e3e; padding: 0.5em; border-radius: 1em;"><div style="line-height: 1.2;"><span style="color: #ffffff"><div style="text-align: center;">Добавляйтесь в нашу группу<br> ПлагінЫ БЕЗ ЦЕНЗУРЫ</br><span style="color: #f3d900">@lampa_plugins_uncensored</span></span></div></div></div></div>'	
...
if ($('DIV[data-name="' + itemName + '"]').find('.settings-param__status').hasClass('active')) {Lampa.Noty.show("Плагін уже установлен!")} else {	
	// Если перезагрузки не требуется - контроль после удаления Плагінов
   if (!Lampa.Storage.get('needReboot')) {
	// Получаем список Плагінов
...
// Отправляем сигнал ожидания выхода из настроек для появления окна с предложением перезагрузки
	  ...
	  // settingsWatch();
   } //else {showReload('Для установки Плагінов после удаления, нужно перезагрузить приложение');}
}
}	
...
/* Интерфейс */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін позволяет на выбор Встановити различные дополнения (скринсейверы, стилизация кнопок, стиль плеера, часы в плеере и т.п.)'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін возвращает в главное меню старый стиль отображения пунктов (Закладки, Нравится, Позже)'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін сбрасывает настройки субтитров по умолчанию'
					},
					onChange: function(value) {
						if (value == '1') {
	         ...
	         Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Мультфильмы',
						description: 'Плагін заменяет пункт Аниме на Мульт'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Погода',
						description: 'Плагін будет поочередно показывать время и погоду, чередуя их показания'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Cub Off',
						description: 'Плагін убирает элементы, предлагающие оформить cub premium'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін позволяет добавить на выбор в главное меню категории (Документалки, Концерты и Мультфильмы)'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін добавляет дополнительные источники для получения информации о фильмах'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Start',
						description: 'Плагін позволяет заходить на заблокированные карточки'
					},
					onChange: function(value) {
						if (value == '1') {
	          ...
	          Lampa.SettingsApi.addParam({
						...
						field: {
							name: 'Золотая тема',
							description: 'Плагін включает золотую тему'
						},
						onChange: function(value) {
							if (value == '1') {
...
/* Управление */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
                                    name: 'Выход',
                                    description: 'Плагін добавляет пункт Выход в главное меню'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					//default: '1',
                            		...
                            		field: {
                                    		name: 'Горячие кнопки',
                                    		description: 'Плагін вызывает меню плеера лампы по кнопкам пульта: 5 - плейлист, 8 - аудиодорожки, 0 - субтитры, channel+/- следующий/предыдущий файл в плейлисте'
                            		},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
                                    		...
                                    		description: 'Плагін работает на устройстве Orsay, для Tizen необходимо обновить виджет до версии 1.9.1'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'Тип управления',
                                    		description: 'Плагін при установке позволяет выбрать тип управления (Пульт без мышки, Пульт с мышью, Тачскрин), а так же сбросить тип управления выбранный ранее'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		...
                                    		description: 'Плагін выключает сенсорное управление, если оно было включено по ошибке'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'Wsoff',
                                    		description: 'Плагін отключения ошибки (Request was denied for security) на старых версиях Android.Не устанавливать, если ошибки нет'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'Смена сервера',
                                    		description: 'Плагін позволяет сменить сервер приложения'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Онлайн */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Modss',
						description: 'Плагін позволяет смотреть фильмы и сериалы в онлайн. На выбор доступно 17 балансеров и различные дополнения через меню настроек Modss. VIP 4K можно подключить через телеграм-бота @modssmy_bot'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Online_Mod',
						description: 'Плагін позволяет смотреть фильмы и сериалы в онлайн. На выбор доступно 7 балансеров'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Аналог Плагіна Online от разработчика приложения Lampa, но с новым информативным интерфейсом для просмотра фильмов и сериалов в онлайн'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
                                  		name: 'Онлайн BWA',
                                  		description: 'Плагін для просмотра фильмов и сериалов в онлайн'
					},
                           		onChange: function(value) {
					if (value == '1') {
...
/* Торрент */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін позволяет переключаться между парсерами jackett из списка с уже забитыми правильными параметрами. В настройках парсера появится пункт со списком общедоступных jacketts'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Tracks',
						description: 'Плагін заменяет название аудиодорожек и субтитров в плеере (работает только в торрентах)'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін для ТВ, на которых Lampa установлена через официальные магазины LG Store и Tizen App Store. Включает в настройках отображение пунктов Парсер и Torrserver, необходимых для просмотра торрентов'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін позволяет произвести поиск вашего локального TorrServera'
					},
					onChange: function(value) {
						if (value == '1') {
	         ...
	         Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плагін добавляет подключение торрент клиентов таких как qBittorent, Transmission с последующей возможностью загрузки торрента через него для локального просмотра'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Телеканалы */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Дизель ТВ',
						description: 'Плагін для бесплатного просмотра телеканалов и коммерческих плейлистов с телепрограммой и записью архива'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Kulik',
						description: 'Плагін для просмотра IPTV каналов, отсортированных по различным категориям. Есть возможность поменять стиль Плагіна, сервер вещания, а также добавить каналы в избранное'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
							name: 'IPTV',
							description: 'Плагін для просмотра IPTV каналов. Сортировка каналов по группам и возможность добавить каналы в избранное. Работает только со своим плейлистом, добавленным на сайте https://cub.watch/iptv'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
							name: 'Hack TV',
							description: 'Плагін для просмотра IPTV каналов'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Радио */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Всё тот же Плагін Радио Record, но с единым списком станций без разделения на жанры'
					},
					onChange: function(value) {
                        			if (value == '1') {
...
/* Клубника */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Клубничка',
						description: 'Плагін для просмотра контента 18+'
					},
					onChange: function(value) {
						if (value == '1') {
	       ...
	       /* Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Sisi Nyam',
						description: 'Другая версия Плагіна Клубничка (частично платная)'
					},
					onChange: function(value) {
						if (value == '1') {
